#!/usr/bin/env python3
"""
pdf/rotate.py — PDF page rotation script
Rotates pages by 90, 180, or 270 degrees, optionally on a page subset.
"""

import argparse
import sys
from pathlib import Path

try:
    from pypdf import PdfReader, PdfWriter
except ImportError:
    print("pypdf is required: pip install pypdf", file=sys.stderr)
    sys.exit(1)


SUPPORTED_FORMATS = {".pdf"}


def parse_page_ranges(spec: str, total: int) -> list[int]:
    """Parse "1-3,5,7-9" → sorted list of 0-based page indices."""
    indices: set[int] = set()
    for part in spec.split(","):
        part = part.strip()
        if "-" in part:
            start_s, end_s = part.split("-", 1)
            start, end = int(start_s), int(end_s)
            if start < 1 or end > total or start > end:
                raise ValueError(f"Page range {part!r} is out of bounds (1–{total})")
            indices.update(range(start - 1, end))
        else:
            page = int(part)
            if page < 1 or page > total:
                raise ValueError(f"Page {page} is out of bounds (1–{total})")
            indices.add(page - 1)
    return sorted(indices)


def rotate_pdf(src: Path, dest: Path, angle: int, pages_spec: str | None) -> int:
    """Rotate pages in src and write to dest. Returns count of rotated pages."""
    reader = PdfReader(str(src))
    writer = PdfWriter()
    total = len(reader.pages)
    targets = set(parse_page_ranges(pages_spec, total)) if pages_spec else set(range(total))

    rotated = 0
    for i, page in enumerate(reader.pages):
        if i in targets:
            page.rotate(angle)
            rotated += 1
        writer.add_page(page)

    with open(dest, "wb") as fh:
        writer.write(fh)

    return rotated


def main():
    parser = argparse.ArgumentParser(
        description="Rotate PDF pages by 90, 180, or 270 degrees."
    )
    parser.add_argument(
        "input",
        nargs="+",
        type=Path,
        help="Input PDF file(s) or director(ies).",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
        required=True,
        help="Output directory for rotated PDFs.",
    )
    parser.add_argument(
        "--angle",
        type=int,
        choices=[90, 180, 270],
        required=True,
        help="Rotation angle in degrees (clockwise).",
    )
    parser.add_argument(
        "--pages",
        metavar="RANGE",
        help='Page range to rotate, e.g. "1-3,5". Default: all pages.',
    )
    parser.add_argument(
        "--recursive",
        "-r",
        action="store_true",
        help="Recurse into subdirectories.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Show what would be done without writing files.",
    )

    args = parser.parse_args()

    files: list[Path] = []
    for inp in args.input:
        if inp.is_dir():
            glob = inp.rglob("*") if args.recursive else inp.glob("*")
            files.extend(f for f in glob if f.suffix.lower() in SUPPORTED_FORMATS)
        elif inp.is_file():
            if inp.suffix.lower() in SUPPORTED_FORMATS:
                files.append(inp)
            else:
                print(f"Skipping unsupported file: {inp}", file=sys.stderr)
        else:
            print(f"Not found: {inp}", file=sys.stderr)

    if not files:
        print("No PDF files found.", file=sys.stderr)
        sys.exit(1)

    if not args.dry_run:
        args.output.mkdir(parents=True, exist_ok=True)

    total_rotated = 0
    errors = 0

    for src in files:
        dest = args.output / src.name

        if args.dry_run:
            scope = f"pages={args.pages}" if args.pages else "all pages"
            print(f"[dry-run] {src} → {dest} (rotate {args.angle}°, {scope})")
            continue

        try:
            count = rotate_pdf(src, dest, args.angle, args.pages)
            total_rotated += count
            print(f"  {src.name}: rotated {count} page(s) by {args.angle}° → {dest}")
        except Exception as e:
            print(f"  ERROR {src.name}: {e}", file=sys.stderr)
            errors += 1

    if not args.dry_run and files:
        print(
            f"\nTotal: {total_rotated} page(s) rotated | "
            f"{len(files) - errors} ok, {errors} errors"
        )


if __name__ == "__main__":
    main()
